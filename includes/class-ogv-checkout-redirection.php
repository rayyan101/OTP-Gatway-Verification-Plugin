...
if ( ! class_exists( 'OGV_Checkout_Redirection' ) ) {
	/**
	 * Class OVG_Checkout_Redirection
	 */
	class OGV_Checkout_Redirection {
		...
		public function otp_gateway_order_received_redirect() {
			...
			if ( 'otp_gatway' === $order->get_payment_method() && empty( $verified ) ) {
				// OTP Verification Gateway, redirects to a OTP Verification form.
				wp_safe_redirect( esc_url( 'class-ogv-otp-checking.php' ) );
				die;
			}
